...
class Viterbi(object):
    ...
    def __init__(self, secoHmm):
        '''
        Constructor
        '''
        
        self.secoHmm = secoHmm

    def computeBestPath(self, oseq):
    
        delta = np.zeros( (oseq.shape[0], self.secoHmm.stateSize, self.secoHmm.stateSize) )
        psy = np.zeros( (oseq.shape[0], self.secoHmm.stateSize, self.secoHmm.stateSize) )
            
        stateSequence = np.zeros( oseq.shape[0], dtype='int')
            
        oseqInd = range( len(oseq) )
        stateInd = range( self.secoHmm.stateSize )
        T = len(oseq)-1
        ...
        def _argMax(t, j, k, s_i):
            ...
            for i in stateInd[s_i:]:
                ...
                tmp = delta[t,i,j] * self.secoHmm.getAijk(i, j, k)
            ...
            for t in oseqInd[1:]:   # Compute through observables.
                ...
                for j in stateInd[1:]:
                    for k in stateInd[1:]:
                        ...
                        delta[t, j, k] = delta[t, j, k] * self.secoHmm.getBijk(j, k, o_t)
                s_i = 1
    ...
    #   Initialization, delta has been initialized to zeros
        ...
        for i in stateInd:
            #print self.secoHmm.getPii(i)
            delta[0, 0, i] = self.secoHmm.getPii(i) * self.secoHmm.getBijk(0, i, oseq[0]) # pi(i) * b_ij0
            psy[0, 0, i] = 0
